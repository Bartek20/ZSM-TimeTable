...
const updateBroadcaster = {
  cacheDidUpdate: async ({
    ...
    if (!(oldResponse instanceof Response && newResponse instanceof Response)) {
      return
    }
    ...
    if (file == 'lista.html') {
      ...
      const genOld = /wygenerowano (\d{1,4}[./-]\d{1,2}[./-]\d{1,4})/.exec(
        oldText
      )
      const genNew = /wygenerowano (\d{1,4}[./-]\d{1,2}[./-]\d{1,4})/.exec(
        newText
      )
...
export default {
  ...
  workbox: {
    ...
    globIgnores: [
      'schoolData.js',
      'schoolData.template.js',
      'timetableData.js',
      'timetableData.template.js'
    ],
