...
<template>
	<div class="lesson" v-tooltip.top="{
		...
		<div class="row">
			...
			<div v-if="lessonData.columns.left.id">
				<RouterLink
					:to="{ name: 'plan', params: { user: user, mode: lessonData.columns.left.mode, id: lessonData.columns.left.id } }"
					class="text-muted text-decoration-none">{{
						lessonData.columns.left.name
					}}</RouterLink>
			</div>
			<div v-else-if="!lessonData.columns.left.id && lessonData.columns.left.name">{{ lessonData.columns.left.name }}
			</div>
			<div v-else>-</div>
			<!-- Column #2 -->
			<div v-if="lessonData.columns.right.id">
				<RouterLink
					:to="{ name: 'plan', params: { user: user, mode: lessonData.columns.right.mode, id: lessonData.columns.right.id } }"
					class="text-muted text-decoration-none">{{
						lessonData.columns.right.name
					}}</RouterLink>
			</div>
			<div v-else-if="!lessonData.columns.right.id && lessonData.columns.right.name">{{ lessonData.columns.right.name }}
			</div>
			<div v-else>-</div>
		</div>
	</div>
	<span class="lessonOldData" v-else>
		...
		&nbsp;
		<!-- Column #1 -->
		<RouterLink v-if="lessonData.columns.left.id"
			:to="{ name: 'plan', params: { user: user, mode: lessonData.columns.left.mode, id: lessonData.columns.left.id } }">
		...
		&nbsp;
		<!-- Column #2 -->
		<RouterLink v-if="lessonData.columns.right.id"
			:to="{ name: 'plan', params: { user: user, mode: lessonData.columns.right.mode, id: lessonData.columns.right.id } }"
			class="text-muted text-decoration-none">{{
				lessonData.columns.right.name
			}}</RouterLink>
