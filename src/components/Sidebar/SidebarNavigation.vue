...
const searchData = computed(() => {
  ...
  if (Object.keys(appData.list.value).length > 0) {
    appData.list.value.classes.forEach((item) => {
      const out = {};
      if (!appConfigs.value.database.classes[item.name])
        parseName("o", item.name);
    ...
    appData.list.value.teachers.forEach((item) => {
      const out = {};
      if (!appConfigs.value.database.teachers[item.name])
        parseName("n", item.name);
    ...
    appData.list.value.rooms.forEach((item) => {
      const out = {};
      if (!appConfigs.value.database.rooms[item.name])
        parseName("s", item.name);
  ...
  if (search.value.query.length === 0)
    return {
      ...
      teachers:
        ...
        user.value === "uczen"
          ? []
          : searchData.value.teachers,
      rooms:
        !appConfigs.value.school.allowStrudentsViewRooms &&
        user.value === "uczen"
          ? []
          : searchData.value.rooms,
    };
    ...
    teachers:
      ...
      user.value === "uczen"
        ...
        : teacherSearcher
            ...
            .map((item) => ({ ...item.item })),
    rooms:
      !appConfigs.value.school.allowStrudentsViewRooms && user.value === "uczen"
        ? []
        : roomSearcher
            .value(search.value.query)
            .map((item) => ({ ...item.item })),
