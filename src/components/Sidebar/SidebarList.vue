...
function changeMenu(e) {
  ...
  if (element.tagName !== "LABEL") element = element.parentElement;
  if (!document.querySelector(".sidebar__menu--checkbox:checked"))
    element.parentElement.classList.add("only");
  ...
  <div class="sidebar__menu" v-if="list.length > 0">
    ...
    <div class="sidebar__menu__list" @transitionend="transitioned">
      <ul class="sidebar__menu__list__container">
        <li
          class="sidebar__menu__list__container__item"
          v-for="item in list"
          :key="`${mode}/${item.id}`"
        >
          <RouterLink
            class="sidebar__menu__list__container__item__link"
            :to="{ name: 'plan', params: { mode: mode, id: item.id } }"
          >
            <span class="sidebar__menu__list__container__item__link__text">{{
              item.name
            }}</span>
          </RouterLink>
        </li>
