...
export default function parseData(obj, data) {
  if (
    ...
    log(
      "error",
      "[App] Brak danych do przetworzenia lub nieprawidłowy obiekt danych",
    );
    return;
  }
  const diff = [];
  ...
  oldData.forEach((key) => {
    if (!newData.includes(key)) {
      ...
      if (obj === "subjects") {
        appConfigs.value.database.subjects[key.replace(/ \([UR]{1}\)/, "")] =
          undefined;
      }
    }
  });
  newData.forEach((key) => {
    let isDiff = false;
    if (
      (obj === "levels" || obj === "classes") &&
      data[key] !== appConfigs.value.timetable[obj][key]
    ) {
      isDiff = true;
    }
    if (
