...
export default function parseData (obj, data) {
  ...
  if (obj === 'shortLessons') {
    if (
      appConfigs.value.timetable.shortLessons.length !== data.length ||
			appConfigs.value.timetable.shortLessons.some((v, i) => v.number !== data[i].number || v.timeFrom !== data[i].timeFrom || v.timeTo !== data[i].timeTo)
    ) {
  ...
  oldData.forEach((key) => {
    if (!newData.includes(key)) {
      ...
      if (obj === 'teachers' || obj === 'rooms') {
        appConfigs.value.database[obj][key] = undefined
      }
      if (obj === 'subjects') {
        appConfigs.value.database.subjects[key.replace(/ \([UR]{1}\)/, '')] = undefined
      }
    }
  })
  newData.forEach((key) => {
    let isDiff = false
    if ((obj === 'levels' || obj === 'classes') && data[key] !== appConfigs.value.timetable[obj][key]) {
      isDiff = true
    }
    if (
      obj === 'teachers' &&
    ...
    if (
      obj === 'rooms' &&
			(data[key].name !== appConfigs.value.timetable.rooms[key]?.name || data[key].level !== appConfigs.value.timetable.rooms[key]?.level)
    ) {
      isDiff = true
    }
    if (
      obj === 'subjects' &&
			(data[key].short !== appConfigs.value.timetable.subjects[key]?.short || data[key].full !== appConfigs.value.timetable.subjects[key]?.full)
    ) {
      isDiff = true
    }
    if (isDiff) {
